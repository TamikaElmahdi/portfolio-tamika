<template>
  <AdminLayout title="Modifier le témoignage" subtitle="Mettre à jour une recommandation">
    <div class="max-w-2xl">
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid sm:grid-cols-2 gap-6">
            <div class="sm:col-span-2">
              ...
              <input v-model="form.author_name" type="text" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Poste</label>
              <input v-model="form.author_position" type="text" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Entreprise</label>
              <input v-model="form.author_company" type="text" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
            ...
            <div class="sm:col-span-2">
              ...
              <textarea v-model="form.content_fr" rows="4" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"></textarea>
            </div>
            <div class="sm:col-span-2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">Témoignage (EN)</label>
              <textarea v-model="form.content_en" rows="4" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"></textarea>
            </div>
            <div class="sm:col-span-2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">Photo (laisser vide pour conserver l'actuelle)</label>
              <div v-if="testimonial.author_avatar" class="flex items-center gap-3 mb-3">
                <img :src="'/storage/' + testimonial.author_avatar" alt="Avatar" class="w-12 h-12 rounded-full object-cover border border-gray-200" />
...
const form = useForm({
  ...
  author_position: props.testimonial.author_position || '',
  author_company: props.testimonial.author_company || '',
  author_avatar: null,
  content_fr: props.testimonial.content_fr,
  content_en: props.testimonial.content_en || '',
  rating: props.testimonial.rating,
  relation: props.testimonial.relation || '',
  linkedin_url: props.testimonial.linkedin_url || '',
  featured: props.testimonial.featured,
  order: props.testimonial.order,
  _method: 'PUT',
})

const submit = () => form.post('/admin/testimonials/' + props.testimonial.id, { forceFormData: true })
</script>
