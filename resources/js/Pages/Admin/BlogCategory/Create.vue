<template>
  <AdminLayout title="Nouvelle catégorie" subtitle="Créer une catégorie de blog">
    <div class="max-w-lg">
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Slug (laisser vide pour auto-génération)</label>
            <input v-model="form.slug" type="text" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 font-mono text-sm" placeholder="ex : developpement-web" />
          ...
          <div>
            ...
            <div class="flex items-center gap-3">
              ...
              <input v-model="form.color" type="text" class="flex-1 px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 font-mono text-sm" placeholder="#3B82F6" />
            </div>
...
const form = useForm({
  name_fr: '', name_en: '', slug: '', color: '#3B82F6', order: 0,
})

const submit = () => form.post('/admin/blog-categories')
</script>
