<template>
  <AdminLayout title="Nouvelle formation" subtitle="Ajouter une formation académique">
    <div class="max-w-3xl">
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input v-model.number="form.start_year" type="number" required min="1990" max="2030" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Année de fin</label>
              <input v-model.number="form.end_year" type="number" min="1990" max="2030" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div class="sm:col-span-2">
...
const form = useForm({ school: '', degree_fr: '', degree_en: '', field_fr: '', field_en: '', description_fr: '', description_en: '', start_year: new Date().getFullYear(), end_year: null, order: 0 })
const submit = () => form.post('/admin/educations')
</script>
