<template>
  <AdminLayout :title="'Modifier : ' + post.title_fr" subtitle="Modifier un article de blog">
    <div class="max-w-4xl">
          ...
          <div>
            ...
            <div class="grid sm:grid-cols-2 gap-6">
              <div>
                ...
                <textarea v-model="form.excerpt_fr" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"></textarea>
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Extrait (EN)</label>
                <textarea v-model="form.excerpt_en" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"></textarea>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-4">
              <div>
                ...
                <textarea v-model="form.content_fr" rows="14" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-y font-mono text-sm"></textarea>
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Contenu (EN)</label>
                <textarea v-model="form.content_en" rows="14" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 resize-y font-mono text-sm"></textarea>
              </div>
            </div>
          </div>
            ...
            <div class="grid sm:grid-cols-2 gap-6">
              <div>
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center cursor-pointer hover:border-teal-400 hover:bg-teal-50/30 transition-colors" @click="coverInput.click()">
                  ...
                  <img v-else-if="post.cover_image" :src="'/storage/' + post.cover_image" class="w-full h-32 object-cover rounded-lg mb-2" />
                  <div v-else class="text-gray-400 text-sm">📷 Cliquez pour changer</div>
                  <p v-if="post.cover_image" class="text-xs text-gray-400 mt-1">Cliquer pour remplacer</p>
                </div>
                <input ref="coverInput" type="file" accept="image/*" class="hidden" @change="onCoverChange" />
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Image Open Graph</label>
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center cursor-pointer hover:border-teal-400 hover:bg-teal-50/30 transition-colors" @click="ogInput.click()">
                  ...
                  <img v-else-if="post.og_image" :src="'/storage/' + post.og_image" class="w-full h-32 object-cover rounded-lg mb-2" />
                  <div v-else class="text-gray-400 text-sm">🌐 Cliquez pour ajouter</div>
                </div>
          ...
          <div>
            ...
            <div class="grid sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input v-model="form.tags" type="text" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500" />
...
const form = useForm({
  ...
  published_at: formatForInput(props.post.published_at),
  meta_title_fr: props.post.meta_title_fr || '',
  meta_title_en: props.post.meta_title_en || '',
  meta_description_fr: props.post.meta_description_fr || '',
  meta_description_en: props.post.meta_description_en || '',
  order: props.post.order || 0,
...
const submit = () => form.post('/admin/blog/' + props.post.id, { forceFormData: true })
</script>
